...
const permissionMiddlewareCreator = new PermissionMiddlewareCreator('items');

// This file contains the logic of every route in Forest Admin for the collection items:
...
// Delete a Item
router.delete('/items/:recordId', permissionMiddlewareCreator.delete(), (request, response, next) => {
...
// Get a Item
router.get('/items/:recordId', permissionMiddlewareCreator.details(), (request, response, next) => {
...
router.get('/items.csv', permissionMiddlewareCreator.export(), (request, response, next) => {
